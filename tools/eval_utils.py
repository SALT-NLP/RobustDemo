...
def macro_score(y_true, y_pred, digits=2, suffix=False):
	...
	ps, rs, f1s, s = [], [], [], []
	for type_name, true_entities in sorted(d1.items()):
		pred_entities = d2[type_name]
		nb_correct = len(true_entities & pred_entities)
		nb_pred = len(pred_entities)
		nb_true = len(true_entities)
...
def get_error_types(y_true, y_pred):
	...
	true_entities_rest = true_entities - correct
	pred_entities_rest = pred_entities - correct
	...
	complete_false_positive = pred_entities_rest - set([item[1] for item in right_label_overlapping_span]) - set([item[1] for item in wrong_label_overlapping_span]) - set([item[1] for item in wrong_label_right_span])
	...
	results = {
		...
		'n_complete_false_positive': len(complete_false_positive),
		'n_complete_false_negative': len(complete_false_negative),
	}
	return results
